...
function matchHyperlink(text, protocols, col = 0) {
	...
	console.log("67", nextMatch);
	if (nextMatch) {
		return nextMatch;
	}
	if (match.index + match[0].length >= col) {
		console.log("72", match);
		return match[0];
	}
	return null;
}
	...
	getProvider() {
		return {
			...
			getSuggestionForWord: (textEditor, text, range) => {
				console.log("162", textEditor, text, range);
				const {link, linkText} = getHyperlink(textEditor, range, this.protocols);
				console.log("164", link, linkText);
				if (!link) {
					return null;
				}
