...
function matchHyperlink(text, protocols, col = 0) {
	...
	console.log("hh 67", nextMatch);
	if (nextMatch) {
		return nextMatch;
	}
	if (match.index + match[0].length >= col) {
		console.log("hh 72", match);
		return match[0];
	}
	return null;
}
...
function getHyperlink(textEditor, range, protocols) {
	...
	if (matched) {
		...
		if (token && token.value) {
			...
			console.log("hh 101", link, linkText);
			if (link) {
				return {
					link,
					linkText,
	...
	getProvider() {
		return {
			...
			getSuggestionForWord: (textEditor, text, range) => {
				console.log("hh 162", textEditor, text, range);
				const {link, linkText} = getHyperlink(textEditor, range, this.protocols);
				console.log("hh 164", link, linkText);
				if (!link) {
					return null;
				}
